...
const menuItems = [
  ...
  { name: "PNP Fiyatları", href: "/admin/pnp-pricing", icon: "M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" },
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 items-start">
      {/* Sidebar */}
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col shrink-0 sticky top-0 h-screen z-20">
        <div className="flex items-center px-6 py-6 border-b border-slate-800">
          <Link href="/admin" className="text-lg font-black text-white tracking-wide leading-tight">
            BASKI ATÖLYESİ <br />
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors font-medium ${isActive ? "bg-slate-800 text-white" : "hover:bg-slate-800/50 hover:text-white"
                  }`}
              >
      ...
      <main className="flex-1 flex flex-col min-h-screen">
        {/* Header */}
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-end px-8 shrink-0 sticky top-0 z-10">
